...
--    ne_comm               : a ≠ b ↔ b ≠ a
--    mul_lt_mul_iff_right₀ : 0 < a → (a * b < a * c ↔ b < c)
--    mul_lt_mul_iff_left₀  : 0 < a → (b * a < c * a ↔ b < c)
--    mul_le_mul_iff_left₀  : 0 < a → (b * a ≤ c * a ↔ b ≤ c)
--    one_mul               : 1 * a = a
--
...
-- y, por tanto,
--    |x * y| = |x| * |y|    [por abs_mul]
--            < |x| * ε      [por mul_lt_mul_iff_right₀, (3) y (hy)]
--            < ε * ε        [por mul_lt_mul_iff_left₀, (he1) y (hx)]
--            ≤ 1 * ε        [por mul_le_mul_iff_left₀, (he1) y (he2)]
--            = ε            [por one_mul]
...
by
  ...
  . -- h : ¬|x| = 0
    ...
    calc |x * y|
         = |x| * |y| := abs_mul x y
       _ < |x| * ε   := (mul_lt_mul_iff_right₀ h1).mpr hy
       _ < ε * ε     := (mul_lt_mul_iff_left₀ he1).mpr hx
       _ ≤ 1 * ε     := (mul_le_mul_iff_left₀ he1).mpr he2
       _ = ε         := one_mul ε
  ...
  . -- h : ¬|x| = 0
    ...
    calc
      |x * y| = |x| * |y| := by rw [abs_mul]
            _ < |x| * ε   := by apply (mul_lt_mul_iff_right₀ h1).mpr hy
            _ < ε * ε     := by apply (mul_lt_mul_iff_left₀ he1).mpr hx
            _ ≤ 1 * ε     := by apply (mul_le_mul_iff_left₀ he1).mpr he2
            _ = ε         := by rw [one_mul]
  ...
  . -- h : ¬|x| = 0
    ...
    calc
      |x * y| = |x| * |y| := by simp [abs_mul]
            _ < |x| * ε   := by simp only [mul_lt_mul_iff_right₀, h1, hy]
            _ < ε * ε     := by simp only [mul_lt_mul_iff_left₀, he1, hx]
            _ ≤ 1 * ε     := by simp only [mul_le_mul_iff_left₀, he1, he2]
...
#check (mul_le_mul_iff_left₀ : 0 < a → (b * a ≤ c * a ↔ b ≤ c))
#check (mul_lt_mul_iff_right₀ : 0 < a → (a * b < a * c ↔ b < c))
#check (mul_lt_mul_iff_left₀ : 0 < a → (b * a < c * a ↔ b < c))
